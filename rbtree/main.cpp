...
struct tree{
	...
	struct node{//node stuff
		...
		int data;//#1-999
		char col;//r/b red/black
		node(int data, char col, node* prev = nullptr, node* left = nullptr, node* right = nullptr) : data(data), col(col), prev(prev), left(left), right(right) {}//inline constructor defaults to no neighbors/linked nodes
	...
	node* root;
	tree(node* _root) : root(_root) {}//tree constructor
	...
	void print(node*& root, int depth = 0){//print tree sideways with tab spacing
		...
		for(int i=0; i<depth; i++){std::cout<<"          ";}//depth spacing
			std::cout<<"("<<root->prev->data<<")"<<"("<<root->col<<")"<<root->data<<'\n';//depth-based tab spacing
	...
	void insert(node*& curr, node*& prev, int numInput){
		if(curr == nullptr){//base case, once "root" is nullptr, add the node, and if this is not the first node, update the previous pointer.
			curr = new node(numInput, 'R',nullptr,nullptr,nullptr);
...
Items:
Rebalance must be called after EVERY insertion &/or deletion
Add needs to be modified to prompt input method
Change input splitting? Not necessary

Rules:
All new nodes are red and insterted by BST rule(s)
